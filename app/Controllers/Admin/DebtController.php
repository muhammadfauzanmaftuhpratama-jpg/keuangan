...
class DebtController extends BaseController {
    ...
    public function index(){
        ...
        return view('admin/debt/index',['title'=>'Hutang','debts'=>$rows,'totalDebt'=>$this->dm->getTotalDebt($uid),'search'=>$search,'status'=>$status,'pager'=>$this->dm->pager,'perPage'=>$pp,'totalData'=>$total]);
    }
    public function create(){return view('admin/debt/create',['title'=>'Tambah Hutang']);}
    public function store(){
        ...
        return redirect()->to('/admin/debt')->with('success','Hutang berhasil ditambahkan!');
    }
    public function detail(int $id){
        $d=$this->dm->where('id',$id)->where('user_id',$this->uid())->first();if(!$d)return redirect()->to('/admin/debt')->with('error','Data tidak ditemukan!');
        $pp=10;$p=$this->pm->where('debt_id',$id)->orderBy('payment_date','DESC')->paginate($pp,'payments');
        return view('admin/debt/detail',['title'=>'Detail Hutang','debt'=>$d,'payments'=>$p,'pager'=>$this->pm->pager,'perPage'=>$pp]);
    }
    public function edit(int $id){$d=$this->dm->where('id',$id)->where('user_id',$this->uid())->first();if(!$d)return redirect()->to('/admin/debt')->with('error','Data tidak ditemukan!');return view('admin/debt/edit',['title'=>'Edit Hutang','debt'=>$d]);}
    public function update(int $id){
        if(!$this->dm->where('id',$id)->where('user_id',$this->uid())->first())return redirect()->to('/admin/debt')->with('error','Data tidak ditemukan!');
        ...
        return redirect()->to('/admin/debt')->with('success','Hutang berhasil diupdate!');
    }
    public function delete(int $id){if(!$this->dm->where('id',$id)->where('user_id',$this->uid())->first())return redirect()->to('/admin/debt')->with('error','Data tidak ditemukan!');$this->dm->delete($id);return redirect()->to('/admin/debt')->with('success','Hutang berhasil dihapus!');}
    public function pay(int $id){
        $d=$this->dm->where('id',$id)->where('user_id',$this->uid())->first();if(!$d)return redirect()->to('/admin/debt')->with('error','Data tidak ditemukan!');
        if($d['status']==='paid')return redirect()->to('/admin/debt/detail/'.$id)->with('error','Hutang ini sudah lunas!');
        ...
        return redirect()->to('/admin/debt/detail/'.$id)->with('success',$msg);
    }
}
