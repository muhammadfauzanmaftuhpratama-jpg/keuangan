...
class CategoryController extends BaseController {
    ...
    public function index()
    {
        ...
        return view('admin/category/index', [
            'title' => 'Kategori',
            'incomeCategories' => $this->cm->getUserCategories($uid, 'income'),
            'expenseCategories' => $this->cm->getUserCategories($uid, 'expense'),
        ]);
    }
    ...
    public function store()
    {
        if (! $this->validate([
            ...
            'type' => ['in_list' => 'Tipe harus income atau expense!'],
            'color' => ['regex_match' => 'Format warna tidak valid!']
        ])) {
        ...
        if ($this->cm->where('user_id', $uid)->where('type', $type)->where('name', $name)->first()) {
        ...
        $this->cm->insert([
            ...
            'icon' => $this->request->getPost('icon') ?: 'bi bi-tag',
    ...
    {
        $c = $this->cm->where('id', $id)->where('user_id', $this->uid())->first();
    ...
    public function update(int $id)
    {
        ...
        if ($this->cm->where('user_id', $uid)->where('type', $c['type'])->where('name', $name)->where('id !=', $id)->first()) {
            return redirect()->back()->withInput()->with('error', 'Kategori dengan nama ini sudah ada!');
        }
    ...
    public function delete(int $id)
    {
        ...
        if ($c['is_default']) {
            return redirect()->to('/admin/category')->with('error', 'Kategori default tidak bisa dihapus!');
        }
