...
class TransactionController extends BaseController {
    ...
    private function listView(string $type){
        ...
        return view('admin/transaction/'.($type==='income'?'income':'expense'),['title'=>$type==='income'?'Pemasukan':'Pengeluaran','transactions'=>$rows,'total'=>$this->t->getTotalByType($uid,$type,$month,$year),'month'=>$month,'year'=>$year,'search'=>$search,'pager'=>$this->t->pager,'perPage'=>$pp,'totalData'=>$total,'categories'=>$this->c->getUserCategories($uid,$type)]);
    }
    public function income(){return $this->listView('income');}
    public function expense(){return $this->listView('expense');}
    public function createIncome(){return view('admin/transaction/create',['title'=>'Tambah Pemasukan','type'=>'income','categories'=>$this->c->getUserCategories($this->uid(),'income')]);}
    public function createExpense(){return view('admin/transaction/create',['title'=>'Tambah Pengeluaran','type'=>'expense','categories'=>$this->c->getUserCategories($this->uid(),'expense')]);}
    public function storeIncome(){return $this->storeData('income','/admin/income');}
    public function storeExpense(){return $this->storeData('expense','/admin/expense');}
    public function editIncome(int $id){return $this->editView($id,'income','/admin/income');}
    public function editExpense(int $id){return $this->editView($id,'expense','/admin/expense');}
    public function updateIncome(int $id){return $this->updateData($id,'income','/admin/income');}
    public function updateExpense(int $id){return $this->updateData($id,'expense','/admin/expense');}
    public function deleteIncome(int $id){return $this->deleteData($id,'/admin/income');}
    public function deleteExpense(int $id){return $this->deleteData($id,'/admin/expense');}
    ...
    private function editView(int $id,string $type,string $redirect){
        ...
        return view('admin/transaction/edit',['title'=>'Edit '.($type==='income'?'Pemasukan':'Pengeluaran'),'type'=>$type,'transaction'=>$trx,'categories'=>$this->c->getUserCategories($this->uid(),$type)]);
    }
