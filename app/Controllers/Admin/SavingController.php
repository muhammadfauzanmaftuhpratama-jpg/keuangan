...
class SavingController extends BaseController {
    ...
    public function index(){
        ...
        return view('admin/saving/index',['title'=>'Tabungan','savings'=>$rows,'totalSaving'=>$this->sm->getTotalSavings($uid),'search'=>$search,'pager'=>$this->sm->pager,'perPage'=>$pp,'totalData'=>$total]);
    }
    public function create(){return view('admin/saving/create',['title'=>'Tambah Tabungan']);}
    public function store(){
        ...
        return redirect()->to('/admin/saving')->with('success','Target tabungan berhasil dibuat!');
    }
    public function detail(int $id){
        $s=$this->sm->where('id',$id)->where('user_id',$this->uid())->first();
        if(!$s)return redirect()->to('/admin/saving')->with('error','Tabungan tidak ditemukan!');
        $pp=10;$h=$this->hm->where('saving_id',$id)->orderBy('created_at','DESC')->paginate($pp,'history');
        return view('admin/saving/detail',['title'=>'Detail Tabungan','saving'=>$s,'histories'=>$h,'pager'=>$this->hm->pager,'perPage'=>$pp]);
    }
    public function edit(int $id){$s=$this->sm->where('id',$id)->where('user_id',$this->uid())->first();if(!$s)return redirect()->to('/admin/saving')->with('error','Tabungan tidak ditemukan!');return view('admin/saving/edit',['title'=>'Edit Tabungan','saving'=>$s]);}
    public function update(int $id){
        if(!$this->sm->where('id',$id)->where('user_id',$this->uid())->first())return redirect()->to('/admin/saving')->with('error','Tabungan tidak ditemukan!');
        ...
        return redirect()->to('/admin/saving')->with('success','Tabungan berhasil diupdate!');
    }
    public function delete(int $id){if(!$this->sm->where('id',$id)->where('user_id',$this->uid())->first())return redirect()->to('/admin/saving')->with('error','Tabungan tidak ditemukan!');$this->sm->delete($id);return redirect()->to('/admin/saving')->with('success','Tabungan berhasil dihapus!');}
    public function deposit(int $id){
        ...
        return redirect()->to('/admin/saving/detail/'.$id)->with('success','Setoran berhasil disimpan!');
    }
    public function withdraw(int $id){
        ...
        return redirect()->to('/admin/saving/detail/'.$id)->with('success','Penarikan berhasil!');
    }
}
