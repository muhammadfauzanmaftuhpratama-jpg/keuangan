...
<div><h5 class="fw-bold mb-1">Pengeluaran</h5><p class="text-muted mb-0 small">Kelola data Pengeluaran Anda</p></div>
<a href="/admin/expense/create" class="btn btn-danger btn-sm"><i class="bi bi-plus-lg me-1"></i>Tambah Pengeluaran</a>
</div>
...
<form method="GET" action="/admin/expense" class="row g-2 align-items-end">
...
<?php if($search): ?><a href="/admin/expense?month=<?= $month ?>&year=<?= $year ?>" class="btn btn-outline-secondary btn-sm"><i class="bi bi-x"></i></a><?php endif; ?>
</div></div>
<div class="col-auto"><button type="submit" class="btn btn-primary btn-sm"><i class="bi bi-funnel me-1"></i>Filter</button></div>
<div class="col-auto ms-auto"><div class="text-end"><div class="small text-muted">Total Pengeluaran</div><div class="fw-bold text-danger"><?= rupiah($total) ?></div></div></div>
</form>
</div>
...
<td><span class="badge bg-danger bg-opacity-10 text-danger"><?= esc($trx['category']) ?></span></td>
<td class="fw-semibold text-danger"><?= $trx['type']==='income'?'+':'-' ?><?= rupiah($trx['amount']) ?></td>
<td class="small"><?= date('d M Y',strtotime($trx['date'])) ?></td>
<td class="text-muted small"><?= $trx['note']?esc($trx['note']):'—' ?></td>
<td>
<a href="/admin/expense/edit/<?= $trx['id'] ?>" class="btn btn-warning btn-sm"><i class="bi bi-pencil"></i></a>
<button class="btn btn-danger btn-sm" onclick="if(confirm('Hapus data ini?'))window.location='/admin/expense/delete/<?= $trx['id'] ?>'"><i class="bi bi-trash"></i></button>
</td>
</tr>
<?php endforeach; else: ?>
<tr><td colspan="6" class="text-center py-4 text-muted"><i class="bi bi-inbox fs-2 d-block mb-2"></i><?= $search?'Tidak ada data yang cocok':'Belum ada data Pengeluaran bulan ini' ?></td></tr>
<?php endif; ?>
</tbody>
</table>
</div>
