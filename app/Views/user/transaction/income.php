...
<a href="/user/income/create" class="btn btn-success btn-sm"><i class="bi bi-plus-lg me-1"></i>Tambah Pemasukan</a>
</div>
...
<form method="GET" action="/user/income" class="row g-2 align-items-end">
...
<?php if($search): ?><a href="/user/income?month=<?= $month ?>&year=<?= $year ?>" class="btn btn-outline-secondary btn-sm"><i class="bi bi-x"></i></a><?php endif; ?>
</div></div>
...
<td>
<a href="/user/income/edit/<?= $trx['id'] ?>" class="btn btn-warning btn-sm"><i class="bi bi-pencil"></i></a>
<button class="btn btn-danger btn-sm" onclick="if(confirm('Hapus data ini?'))window.location='/user/income/delete/<?= $trx['id'] ?>'"><i class="bi bi-trash"></i></button>
</td>
</tr>
<?php endforeach; else: ?>
