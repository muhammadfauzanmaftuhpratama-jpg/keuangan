...
<a href="/user/expense/create" class="btn btn-danger btn-sm"><i class="bi bi-plus-lg me-1"></i>Tambah Pengeluaran</a>
</div>
...
<form method="GET" action="/user/expense" class="row g-2 align-items-end">
...
<?php if($search): ?><a href="/user/expense?month=<?= $month ?>&year=<?= $year ?>" class="btn btn-outline-secondary btn-sm"><i class="bi bi-x"></i></a><?php endif; ?>
</div></div>
...
<td>
<a href="/user/expense/edit/<?= $trx['id'] ?>" class="btn btn-warning btn-sm"><i class="bi bi-pencil"></i></a>
<button class="btn btn-danger btn-sm" onclick="if(confirm('Hapus data ini?'))window.location='/user/expense/delete/<?= $trx['id'] ?>'"><i class="bi bi-trash"></i></button>
</td>
</tr>
<?php endforeach; else: ?>
