...
<a href="/user/debt/create" class="btn btn-danger btn-sm"><i class="bi bi-plus-lg me-1"></i>Tambah Hutang</a>
</div>
...
<form method="GET" action="/user/debt" class="row g-2 align-items-end">
<div class="col-md-5">
...
<?php if($search): ?><a href="/user/debt?status=<?= esc($status) ?>" class="btn btn-outline-secondary"><i class="bi bi-x"></i></a><?php endif; ?>
</div>
</div>
<div class="col-md-3">
...
<td>
<a href="/user/debt/detail/<?= $debt['id'] ?>" class="btn btn-primary btn-sm"><i class="bi bi-eye"></i></a>
<a href="/user/debt/edit/<?= $debt['id'] ?>" class="btn btn-warning btn-sm"><i class="bi bi-pencil"></i></a>
<button class="btn btn-danger btn-sm" onclick="confirmDelete('/user/debt/delete/<?= $debt['id'] ?>', '<?= esc($debt['creditor_name']) ?>')"><i class="bi bi-trash"></i></button>
</td>
</tr>
<?php endforeach; ?>
<?php else: ?>
